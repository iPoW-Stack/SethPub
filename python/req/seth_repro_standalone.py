...
def q_u(client: SethClient, from_hex: str, addr_hex: str, sig: str, types=None, args=None):
    types = types or []
    args = args or []
...
def run_business(client: SethClient, pk: str, sender: str, bridge: str, pool: str, amount_seth: int):
    print("\n=== Business Repro: requestWithdrawToSolanaFromSETH ===")
    bt = q_u(client, sender, bridge, "totalWithdrawRequests()")
    brs = q_u(client, sender, pool, "reserveSETH()")
    bru = q_u(client, sender, pool, "reservesUSDC()")
    print("before totalWithdrawRequests=", bt, "pool=", brs, bru)
    tx = client.send_tx(
        pk, bridge, amount=amount_seth, gas_limit=500000000, gas_price=1, step=8,
        input_hex=encode_call("requestWithdrawToSolanaFromSETH(bytes32,uint256)", ["bytes32", "uint256"], [bytes.fromhex("aa"*32), 1])
    )
    ...
    ars = q_u(client, sender, pool, "reserveSETH()")
    aru = q_u(client, sender, pool, "reservesUSDC()")
    print("after  totalWithdrawRequests=", at, "pool=", ars, aru)


def run_probe_chain(client: SethClient, pk: str, sender: str):
    ...
    b_rs = q_u(client, sender, pool, "reserveSETH()")
    ...
    a_rs = q_u(client, sender, pool, "reserveSETH()")
...
def main():
    ...
    ap.add_argument("--bridge", default="", help="Business repro: SethBridge address")
    ap.add_argument("--pool", default="", help="Business repro: PoolB address")
    ap.add_argument("--amount-seth", type=int, default=100, help="Business repro amount for requestWithdrawToSolanaFromSETH")
