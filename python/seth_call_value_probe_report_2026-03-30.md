# Seth 合约多级调用测试记录（2026-03-30）

## 目标

定位 `requestWithdrawToSolanaFromSETH` 回执成功但状态不变的问题，验证是否为：

- 合约流程代码问题，或
- Seth 节点对“多级合约内调用”执行异常。
...
- `ProbePool.sol`：`sellSETH(minOut)` 更新 `reserveSETH/reserveUSDC`
- `ProbeTreasury.sol`：`swap(minOut)` 内部调用 `pool.sellSETH`
...
- 回执：`status=0`
- 但状态不变：
  - `ProbePool reserveSETH/reserveUSDC: 10000/10000 -> 10000/10000`
  - `ProbeBridge totalRequests: 0 -> 0`
  - `ProbeTreasury totalSwaps: 0 -> 0`

结论：**多级链式调用（Bridge->Treasury->Pool）在当前 Seth 环境表现为“回执成功但状态不落链”**。

---

## 与业务问题的对应

`requestWithdrawToSolanaFromSETH` 的结构与测试二一致（多级合约调用），因此会出现同类现象：
...
1. 将本报告及上述 tx hash 提交 Seth 节点侧排查（重点看 `status=0` 时是否实际执行状态提交）。
