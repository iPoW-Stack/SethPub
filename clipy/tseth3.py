...
contract AMM {
    ...
    function swapToUSDC(
        ...
        wseth.deposit{value: msg.value}();
        IERC20(address(wseth)).approve(address(swapRouter), msg.value);

        swapRouter.exactInputSingle(
            ISwapRouter.ExactInputSingleParams({
                tokenIn: address(wseth),
        ...
        emit WithdrawInitiated(msg.sender, address(wseth), msg.value);
    }
...
def test_token_approve(w3, MY, KEY, token_addr, spender_addr, amount):
    ...
    try:
        ...
        token_contract = w3.seth.contract(address=token_addr, abi=token_abi)
        print(f"    ✅ Contract instance created")
...
def test_router_existing_contract(w3, MY, KEY, token_in, token_out, amount_in):
    ...
    try:
        ...
        router_contract = w3.seth.contract(address=ROUTER_ADDRESS, abi=router_abi)
        print(f"    ✅ Contract instance created")
...
def test_withdraw_to_solana(w3, MY, KEY, token_in, amount_in, fee, amount_out_minimum, solana_recipient):
    ...
    try:
        ...
        contract = w3.seth.contract(address=CONTRACT_ADDRESS, abi=abi)
        print(f"    ✅ Contract instance created")
...
def test_amm_swap_to_usdc(w3, MY, KEY, amount):
    ...
    AMM internally: deposit ETH -> WSETH -> swap via UniV3 -> sUSDC.
    No approve needed - contract handles it internally.
    Workflow:
    1. Deploy AMM(swapRouter, sUSDC, wseth)
    2. Setup prefund
    ...
    WSETH_ADDR  = "421b24de95ee45e21a1023dda694f0a82eb70726"
    FEE         = 2000
    ...
    try:
        ...
        amm = w3.seth.contract(abi=amm_abi, bytecode=amm_bin, sender_address=MY).deploy({
            'from': MY,
            'args': [
                SWAP_ROUTER,
                SUSDC_ADDR,
    ...
    try:
        ...
        quoter = w3.seth.contract(abi=quoter_abi, bytecode=quoter_bin, sender_address=MY).deploy({
            'from': MY,
            'args': [
                FACTORY_ADDR,
                WETH9_ADDR,
...
def test_contract_price(w3, MY, KEY):
    ...
    # initial_fund = 20000
    kill_contract = w3.seth.contract(abi=k_abi1, bytecode=k_bin1, sender_address=MY).deploy({
        'from': MY
    }, KEY)

    # kill_contract = w3.seth.contract(address='1ea1b7f2efe2d6abc776694d8cfc035b41408cbc', abi=k_abi1)

    contract_addr = kill_contract.address
    print(f"Contract deployed at: {contract_addr}")

    # kill_contract = w3.seth.contract(address='0d1853a141c4bc499b7f72ec534a0d36f3629fbe', abi=k_abi)
