...
int TcpConnection::SendPacketWithoutLock(Packet& packet) {
    ...
    if (tcp_state_ == kTcpNone || tcp_state_ == kTcpClosed) {
        SETH_ERROR("bad state, %d, %lu", static_cast<int>(tcp_state_), msg_pkg->msg_id());
        return -1;
    }

    ByteBufferPtr buf_ptr(new ByteBuffer);
    if (!packet_encoder_->Encode(packet, buf_ptr.get())) {
        SETH_ERROR("encode packet failed: %lu", msg_pkg->msg_id());
        return -1;
    }
    ...
    while (true) {
        ...
        if (n < 0) {
            if (errno != EAGAIN && errno != EWOULDBLOCK) {
                SETH_ERROR("write failed on [%d] [%s], %lu", socket_->GetFd(), strerror(errno), msg_pkg->msg_id());
                rc = false;
            }
