...
int Bootstrap::Init(common::Config& config, std::shared_ptr<security::Security>& security) {
    ...
    if (!config.Get("seth", "bootstrap", bootstrap) || bootstrap.empty()) {
        NETWORK_ERROR("config has no seth bootstrap info.");
    ...
    for (uint32_t i = 0; i < split.Count(); ++i) {
        ...
        uint16_t port = 0;
        if (!seth::common::StringUtil::ToUint16(field_split[2], &port)) {
            return kNetworkError;
        }
        ...
        root_bootstrap_.push_back(std::make_shared<seth::dht::Node>(
            kUniversalNetworkId,
...
std::vector<dht::NodePtr> Bootstrap::GetNetworkBootstrap(
        uint32_t network_id,
        uint32_t count) {
    SETH_DEBUG("now get universal dht 1");
