...
int Universal::Join(dht::NodePtr& node) {
    ...
    // add to subnetworks
//     SETH_DEBUG("universal join node: %s:%d", node->public_ip.c_str(), node->public_port);
    ...
    for (auto iter = dht_ptr->begin(); iter < dht_ptr->end(); ++iter) {
        ...
        if (new_item->id_set.find((*iter)->id) != new_item->id_set.end()) {
            if (des_dht != nullptr) {
                ...
                SETH_DEBUG("expand nodes join network %u add new node: %s:%u, %s",
                    sharding_id,
            ...
            if (tmp_shard_id != sharding_id &&
                ...
                SETH_DEBUG("drop universal nodes network %u node: %s:%u, %s",
                    tmp_shard_id,
                    (*iter)->public_ip.c_str(),
                    (*iter)->public_port,
                    common::Encode::HexEncode((*iter)->id).c_str());
    ...
    if (des_dht != nullptr) {
        for (auto iter = old_id_set.begin(); iter != old_id_set.end(); ++iter) {
            ...
            if (fiter == new_item->id_set.end()) {
                des_dht->Drop(*iter);
                SETH_DEBUG("drop nodes network %u node: %s",
                    sharding_id,
    ...
    if (wait_dht != nullptr) {
        ...
        for (auto iter = dht_ptr->begin(); iter < dht_ptr->end(); ++iter) {
            ...
            if (new_item->id_set.find((*iter)->id) != new_item->id_set.end()) {
                ...
                SETH_DEBUG("drop nodes network %u node: %s:%u, %s",
                    sharding_id,
                    (*iter)->public_ip.c_str(),
                    (*iter)->public_port,
                    common::Encode::HexEncode((*iter)->id).c_str());
...
int Universal::AddNodeToUniversal(dht::NodePtr& node) {
    ...
    for (auto sharding_iter = sharding_latest_height_map_.begin();
        ...
        if (id_iter != sharding_iter->second->id_set.end()) {
            ...
            elected = true;
            SETH_DEBUG("universal add node: %s, sharding id: %u",
