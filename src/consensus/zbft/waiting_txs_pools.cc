...
std::shared_ptr<WaitingTxsItem> WaitingTxsPools::LeaderGetValidTxsIdempotently(
    ...
    // SETH_DEBUG("leader get txs coming thread: %d, pool index: %d", thread_id, pool_index);
    #ifdef TEST_NO_CROSS
    ...
    if (txs_item != nullptr) {
        ...
        if (!txs_item->txs.empty()) {
            ...
            SETH_DEBUG("success leader get single txs coming thread: %d, "
        ...
        SETH_DEBUG("failed leader get txs coming thread: %d, pool index: %d, tx count: %d", 
...
std::shared_ptr<WaitingTxsItem> WaitingTxsPools::GetSingleTx(
        ...
        uint32_t pool_index,
        pools::CheckAddrNonceValidFunction addr_nonce_valid_func) {
    SETH_DEBUG("get single tx pool: %u", pool_index);
    std::shared_ptr<WaitingTxsItem> txs_item = nullptr;
    ADD_DEBUG_PROCESS_TIMESTAMP();
    if (pool_index == common::kImmutablePoolSize) {
        SETH_DEBUG("leader get time tx tmblock_tx_ptr: %u", pool_index);
        txs_item = GetTimeblockTx(pool_index, true, addr_nonce_valid_func);
        SETH_DEBUG("GetTimeblockTx: %d", (txs_item != nullptr));
    }
    ...
    if (txs_item == nullptr) {
        ...
        //     if (pool_index == common::kImmutablePoolSize) {
        //         SETH_DEBUG("now get statistic tx leader now GetStatisticTx pool_index: %d", pool_index);
        //     }
        // }
        
        txs_item = GetStatisticTx(pool_index, "");
        SETH_DEBUG("GetStatisticTx: %d", (txs_item != nullptr));
        if (txs_item) {
            auto iter = txs_item->txs.begin();
            uint64_t now_nonce = 0llu;
            if (iter == txs_item->txs.end() || addr_nonce_valid_func(
...
std::shared_ptr<WaitingTxsItem> WaitingTxsPools::GetTimeblockTx(
    ...
    if (tx_ptr != nullptr) {
        ...
        SETH_DEBUG("single tx success to get timeblock tx: tx key: %s, nonce: %lu, unique hash: %s",
            common::Encode::HexEncode(tx_ptr->tx_key).c_str(), 
            tx_ptr->tx_info->nonce(),
            common::Encode::HexEncode(tx_ptr->tx_info->key()).c_str());
        return txs_item;
...
std::shared_ptr<WaitingTxsItem> WaitingTxsPools::GetStatisticTx(
    ...
    if (tx_ptr != nullptr) {
        if (leader) {
            auto now_tm = common::TimeUtils::TimestampUs();
            if (tx_ptr->prev_consensus_tm_us + 300000lu > now_tm) {
                SETH_DEBUG("leader failed get statistic tx.");
                return nullptr;
            }
        ...
        SETH_DEBUG("single tx success get statistic tx %u, %d, tx key: %s, nonce: %lu", 
            pool_index, leader, 
            common::Encode::HexEncode(tx_ptr->tx_key).c_str(),
            tx_ptr->tx_info->nonce());
        return txs_item;
...
std::shared_ptr<WaitingTxsItem> WaitingTxsPools::GetToTxs(
    ...
    if (tx_ptr != nullptr) {
        ...
        SETH_DEBUG("single tx success get to tx %u, is leader: %d, tx key: %s, nonce: %lu", 
            pool_index, leader, 
            common::Encode::HexEncode(tx_ptr->tx_key).c_str(),
            tx_ptr->tx_info->nonce());
        return txs_item;
