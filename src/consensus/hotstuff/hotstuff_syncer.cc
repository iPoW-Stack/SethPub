    ...
    for (uint32_t pool_idx = 0; pool_idx < common::kInvalidPoolIndex; pool_idx++) {
        if (now_us - last_timers_us_[pool_idx] >= SyncTimerCycleUs(pool_idx)) {
            if (common::GlobalInfo::Instance()->pools_with_thread()[pool_idx] == thread_index) {
                // SETH_DEBUG("pool: %d, cur chain: %s, local: %d",
...
Status HotstuffSyncer::SendRequest(uint32_t network_id, view_block::protobuf::ViewBlockSyncMessage& view_block_msg, int32_t node_num) {
    ...
    SETH_DEBUG("now get universal dht 0");
...
Status HotstuffSyncer::processRequest(const transport::MessagePtr& msg_ptr) {
    SETH_DEBUG("handle message hash: %lu", msg_ptr->header.hash64());
...
Status HotstuffSyncer::processRequestSingle(const transport::MessagePtr& msg_ptr) {
    ...
    SETH_DEBUG("pool: %d Send response single, block_size: %lu, network: %lu",
...
Status HotstuffSyncer::ReplyMsg(
    ...
    SETH_DEBUG("pool: %d, network: %lu, ip: %s, port: %d, with_query_hash: %d, hash64: %lu",
...
Status HotstuffSyncer::processResponse(const transport::MessagePtr& msg_ptr) {
    SETH_DEBUG("handle hotstuff syncer message hash64: %lu", msg_ptr->header.hash64());
    auto& view_block_msg = msg_ptr->header.view_block_proto();
...
Status HotstuffSyncer::processResponseQcTc(
    ...
    if (view_block_res.has_high_view_block()) {
        ...
        if (high_view_block->qc().network_id() != common::GlobalInfo::Instance()->network_id() ||
            ...
            SETH_DEBUG("error network id hight qc: %u, hight tc: %u, local: %u",
        ...
        SETH_DEBUG("response received qctc pool_idx: %d, tc: %d, qc: %d",
...
Status HotstuffSyncer::processResponseLatestCommittedBlock(
    ...
    SETH_DEBUG("pool: %d sync latest committed block: %lu", pool_idx, pb_latest_committed_block.qc().view());
...
Status HotstuffSyncer::MergeChain(
    ...
    for (const auto& sync_block : sync_all_blocks) {
        ...
        if (s != Status::kSuccess) {
            SETH_ERROR("pool: %d, merge chain block: %lu failed, s: %d",
    ...
    SETH_DEBUG("0 success new set qc view: %lu, %u_%u_%lu",
...
Status HotstuffSyncer::onRecViewBlock(
    ...
    if (s != Status::kSuccess) {
        SETH_ERROR("pool: %d sync accept failed", pool_idx);
        return s;
    }    
    ...
    if (s != Status::kSuccess) {
        SETH_ERROR("pool: %d store view block failed, hash: %s, view: %lu, cur chain: %s", pool_idx,
