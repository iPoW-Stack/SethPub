...
Status AggCrypto::PartialSign(
    ...
    SETH_DEBUG("partial sign sk: %s, real sk: %s, msg_hash: %s, sig: %s",
...
Status AggCrypto::VerifyAndAggregateSig(
    ...
    // SETH_WARN("====7.0 msg_hash: %s, member_idx: %d", common::Encode::HexEncode(msg_hash).c_str(), member_idx);
...
Status AggCrypto::VerifyMessage(const transport::MessagePtr& msg_ptr) {
    ...
    if (mem_ptr->bls_publick_key == libff::alt_bn128_G2::zero()) {
        SETH_WARN("verify sign failed, backup invalid bls pk: %s",
    ...
    if (security() && security()->Verify(
        ...
        SETH_WARN("verify leader sign failed: %s", common::Encode::HexEncode(mem_ptr->id).c_str());
        return Status::kError;
    }
