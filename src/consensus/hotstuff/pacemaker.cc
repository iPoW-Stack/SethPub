...
void Pacemaker::HandleTimerMessage(const transport::MessagePtr& msg_ptr) {
    // if (IsTimeout()) {
    //     SETH_DEBUG("pool: %d timeout", pool_idx_);
    //     OnLocalTimeout();
    // }
}
...
void Pacemaker::NewAggQc(const std::shared_ptr<AggregateQC>& agg_qc) {
    ...
    if (agg_qc && agg_qc->IsValid()) {
        ...
        if (s != Status::kSuccess) {
            SETH_ERROR("new agg qc failed, pool: %d, s: %d, view: %lu", pool_idx_, s, agg_qc->GetView());
            return;
        }
...
void Pacemaker::OnLocalTimeout() {
    ...
    return;
    // TODO(HT): test
    SETH_DEBUG("OnLocalTimeout pool: %d, view: %d", pool_idx_, CurView());
    // start a new timer for the timeout case
    StopTimeoutTimer();
    duration_->ViewTimeout();
    SETH_DEBUG("local time set start duration is OnLocalTimeout called start timeout: %lu", pool_idx_);
    ...
    if (last_timeout_ && last_timeout_->header.has_hotstuff_timeout_proto() &&
        ...
        SETH_DEBUG("use exist local timeout message pool: %u, "
    ...
    if (crypto_->PartialSign(
        ...
        SETH_ERROR("sign message failed: %u, elect height: %lu, hash: %s",
            common::GlobalInfo::Instance()->network_id(),
            elect_item->ElectHeight(),
    ...
    if (crypto_->PartialSign(
        ...
        SETH_ERROR("sign high qc failed: %u, elect height: %lu, hash: %s",
            common::GlobalInfo::Instance()->network_id(),
            elect_item->ElectHeight(),
    ...
    SETH_DEBUG("now send local timeout msg hash: %s, view: %u, pool: %u, "
...
void Pacemaker::SendTimeout(const std::shared_ptr<transport::TransportMessage>& msg_ptr) {
    ...
    if (leader->index != leader_rotation_->GetLocalMemberIdx()) {
        ...
        SETH_DEBUG("Send TimeoutMsg pool: %d, to ip: %s, port: %d, "
...
void Pacemaker::OnRemoteTimeout(const transport::MessagePtr& msg_ptr) {
    ...
    SETH_DEBUG("====4.0 start pool: %d, view: %d, member: %d, hash64: %lu", 
    ...
    if (timeout_proto.view() < CurView()) {
        SETH_DEBUG("====4.5 over 0 pool: %d, view: %d, curview: %lu, member: %d, hash64: %lu", 
    ...
    Status s = crypto_->VerifyAndAggregateSig(
            ...
            partial_sig,
            agg_sig);
    SETH_DEBUG("====4.0 pool: %d, view: %d, member: %d, status: %d, hash64: %lu", 
    ...
    // New Propose
    if (new_proposal_fn_) {
        SETH_DEBUG("now ontime called propose: %d", pool_idx_);
    ...
    Status s = crypto_->ReconstructAndVerifyThresSign(
        ...
        reconstructed_sign);
    SETH_DEBUG("====4.0.1 pool: %d, view: %d, member: %d, status: %d, hash64: %lu", 
        pool_idx_, timeout_proto.view(), timeout_proto.member_id(), s,
        msg_ptr->header.hash64());
    if (s != Status::kSuccess) {
        SETH_DEBUG("====4.5 over 1 pool: %d, view: %d, member: %d, hash64: %lu, status: %d", 
            pool_idx_, timeout_proto.view(), timeout_proto.member_id(),
            msg_ptr->header.hash64(),
            s);
        return;
    ...
    // 视图切换
    SETH_DEBUG("====4.1 pool: %d, create tc, view: %lu, member: %d, "
        "tc view: %lu, cur view: %lu, high_tc_: %lu",
        pool_idx_, timeout_proto.view(), timeout_proto.member_id(),
        tc.view(), CurView(), high_tc_->view());
    // NewTc(new_tc);
    SETH_DEBUG("====4.1.0 pool: %d, create tc, view: %lu, member: %d, "
        "tc view: %lu, cur view: %lu, high_tc_: %lu",
        pool_idx_, timeout_proto.view(), timeout_proto.member_id(),
        tc.view(), CurView(), high_tc_->view());
    // NewView msg broadcast
    ...
    SETH_DEBUG("====4.1.1 pool: %d, create tc, view: %lu, member: %d, "
        "tc view: %lu, cur view: %lu, high_tc_: %lu",
        pool_idx_, timeout_proto.view(), timeout_proto.member_id(),
        tc.view(), CurView(), high_tc_->view());
    SETH_DEBUG("====4.5 over 2 pool: %d, view: %d, member: %d, hash64: %lu", 
        pool_idx_, timeout_proto.view(), timeout_proto.member_id(),
        msg_ptr->header.hash64());
#endif
}
