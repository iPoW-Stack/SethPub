...
public:
    ...
    Status Verify(
        ...
        if (n == 1) {
            ...
            SETH_WARN("agg sig verify failed, sig.x is %s, msg_hash: %s, member: %d", sig_x_str.c_str(), common::Encode::HexEncode(msg_hash).c_str(), member_idx);
            assert(false);
...
private:
    ...
    Status BatchVerify(
        ...
        for (auto iter = msg_hash_map.begin(); iter != msg_hash_map.end(); iter++) {
            ...
            if (!pk) {
                SETH_ERROR("pool: %d, batch verify failed, pk not found, member_idx: %d",
