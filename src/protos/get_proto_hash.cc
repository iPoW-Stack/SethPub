...
std::string GetElectBlockHash(const elect::protobuf::ElectBlock& elect_block) {
    ...
    for (int32_t i = 0; i < elect_block.in_size(); ++i) {
        ...
        SETH_DEBUG("in: %s, %d", common::Encode::HexEncode(elect_block.in(i).pubkey()).c_str(), mod_num);
    }
    ...
    if (elect_block.has_prev_members()) {
        for (int32_t i = 0; i < elect_block.prev_members().bls_pubkey_size(); ++i) {
            ...
            SETH_DEBUG("pk: %s", common::Encode::HexEncode(pk.x_c0()).c_str());
        }
        ...
        SETH_DEBUG("cpk: %s, prev_elect_height: %lu", common::Encode::HexEncode(pk.x_c0()).c_str(), prev_elect_height);
    }
    ...
    SETH_DEBUG("shard_network_id: %u, elect_height: %lu, hash: %s, string: %s",
