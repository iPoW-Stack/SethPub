    ...
    if (call_mode == kJustCreate || call_mode == kCreateAndCall || call_mode == kCreate2) {
        ...
        *out_res = ThreadLocalEvm().execute(
        ...
        if (call_mode == kJustCreate || call_mode == kCreate2) {
            ...
            SHARDORA_INFO("kJustCreate done: recipient=%s create_addr=%s accounts_size=%zu runtime_sz=%zu",
                common::Encode::HexEncode(to_address).c_str(),
            ...
            if (dbg_it != host.accounts_.end()) {
                SHARDORA_INFO("kJustCreate: recipient has %zu storage entries", dbg_it->second.storage.size());
            } else {
                SHARDORA_INFO("kJustCreate: recipient NOT in accounts_!");
            }
            return kShardoravmSuccess;
        }
