...
ContractArs::ContractArs() : ContractInterface("") {
    ...
    SETH_DEBUG("init paring ars: %s", data);
}
...
void ContractArs::SingleSign(
    ...
    element_snprintf(data, sizeof(data), 
        ...
        y_i);
    SETH_DEBUG("single sign message: %s", data);
}
...
void ContractArs::AggreSign(
    ...
    for (size_t i = 0; i < messages.size(); ++i) {
        ...
        for (auto &y : ring) {
            if (VerifyProof(*pi_i[i], y_primes[i], delta_primes[i], messages[i], ring, y)) {
                proof_valid = true;
                SETH_DEBUG("agg sign verify: %s", element_to_string(agg_signature).c_str());
                break;
            }
        }
    ...
    element_printf("%B\n", agg_signature);
    SETH_DEBUG("agg sign message: %s", element_to_string(agg_signature).c_str());
