...
function get_all_nodes_bls_info(args) {
    PostCode('/shardorahain/get_all_nodes_bls_info/', {
...
function get_tx_list(args) {
    PostCode('/shardorahain/transactions/', {
        'search': "",
        'height': -1,
        'shard': -1,
        'pool': -1,
...
function get_address_info(args) {
    GetCode('/shardorahain/get_balance/' + args[1] + "/");
}

function get_accounts(args) {
    PostCode('/shardorahain/accounts/', {
        'search': "",
        'shard': -1,
        'pool': -1,
...
function get_block_detail(args) {
    GetCode('/shardorahain/get_block_detail/' + args[1] + "/");
}
...
function penc_create_sec_keys(args) {
    PostCode('/shardorahain/penc_create_sec_keys/', {
        "content": "content",
    });
}

function penc_get_sec_keys(id) {
    PostCode('/shardorahain/penc_get_sec_keys/', {
        "id": id,
    });
}
...
function penc_share_new_data(id, content) {
    PostCode('/shardorahain/penc_share_new_data/', {
        "id": id,
        "content": content
    });
}
...
function penc_vote(id, content) {
    PostCode('/shardorahain/penc_vote/', {
        "id": id,
        "group_info": content
    });
}
...
function penc_get_share_data(id, encdata) {
    PostCode('/shardorahain/penc_get_share_data/', {
        "id": id,
        "seckey": "",
        "encdata": encdata,
    });
...
function penc_transactions(type) {
    PostCode('/shardorahain/penc_transactions/', {
...
function ars_create_sec_keys() {
    PostCode('/shardorahain/ars_create_sec_keys/', {});
}

function ars_create_new_vote(username, addr, now_credit, add_credit) {
    PostCode('/shardorahain/ars_create_new_vote/', {
        "username": username,
        "addr": addr,
...
function ars_vote(id, index, group_info) {
    PostCode('/shardorahain/ars_vote/', {
        "id": id,
        "index": index,
...
function ars_transactions(type) {
    PostCode('/shardorahain/ars_transactions/', {
        "type": type,
    });
}
