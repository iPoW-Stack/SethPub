...
async function SetManagerPrefund(contract_address) {
    ...
    var cmd = `clickhouse-client --host 82.156.224.174 --port 9000 -q "select count(distinct(user)) from zjc_ck_prefund_table where contract='${contract_address}' and user in (${check_accounts_str});"`;
...
async function CreateNewSeller(str_prikey) {
    ...
    {
        var contract_address = fs.readFileSync('contract_address', 'utf-8');
        var cmd = `clickhouse-client --host 82.156.224.174 --port 9000 -q  "select prefund from zjc_ck_prefund_table where  contract='${contract_address}' and user='${address}' order by height desc limit 1;"`;
        var try_times = 0;
        while (try_times < 30) {
            try {
            // wait for exec to complete
