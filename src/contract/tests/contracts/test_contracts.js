...
function Prefund(str_prikey, prepay, contract_address) {
...
async function SetManagerPrefund(contract_address) {
    ...
    var check_count = 1;
    var cmd = `clickhouse-client --host 127.0.0.1 --port 9000 -q "select count(distinct(user)) from zjc_ck_prefund_table where contract='${contract_address}' and user in (${check_accounts_str});"`;
