...
// Notify WS subscribers of a terminal tx status (anything except pending states).
static inline void NotifyTxStatus(
        const TxPoolManager::TxStatusCallback& cb,
        const transport::MessagePtr& msg_ptr,
        transport::MessageHandleStatus status) {
    if (!cb || msg_ptr->msg_hash.empty()) return;
    if (status == transport::kMessageHandle || status == transport::kTxAccept) return;
    cb(common::Encode::HexEncode(msg_ptr->msg_hash), status);
}
...
TxPoolManager::TxPoolManager(
    ...
    SETH_DEBUG("TxPoolManager init success: %d", common::kInvalidPoolIndex);
    InitCrossPools();
...
int TxPoolManager::TmpFirewallCheckMessage(const transport::MessagePtr& msg_ptr) {
    ...
    if (tx_msg.pubkey().size() == 64u) {
        ...
        if (msg_ptr->address_info == nullptr) {
            ...
            NotifyTxStatus(tx_status_cb_, msg_ptr, transport::kTxInvalidAddress);
            return transport::kFirewallCheckError;
        }
        ...
        if (oqs.Verify(
            ...
            msg_ptr->handle_status = transport::kTxInvalidSignature;
            NotifyTxStatus(tx_status_cb_, msg_ptr, transport::kTxInvalidSignature);
            return transport::kFirewallCheckError;
        }
...
void TxPoolManager::SyncMinssingRootHeights(uint64_t now_tm_ms) {
    ...
    for (; root_prev_synced_pool_index_ < common::kInvalidPoolIndex; ++root_prev_synced_pool_index_) {
        ...
        if (root_cross_pools_[root_prev_synced_pool_index_].latest_height() == common::kInvalidUint64 ||
            ...
            SETH_DEBUG("max success sync mising heights pool: %u, height: %lu, max height: %lu, des max height: %lu",
...
void TxPoolManager::HandlePoolsMessage(const transport::MessagePtr& msg_ptr) {
    ...
    if (header.has_tx_proto()) {
        ...
        case pools::protobuf::kRootCreateAddress: {
            ...
            SETH_DEBUG("get local tokRootCreateAddress tx message hash: %s, to: %s, amount: %lu nonce: %lu", 
                common::Encode::HexEncode(msg_ptr->msg_hash).c_str(),
                common::Encode::HexEncode(tx_msg.to()).c_str(),
                tx_msg.amount(),
                tx_msg.nonce());
...
void TxPoolManager::SyncPoolsMaxHeight() {
    ...
    for (uint32_t i = network::kRootCongressNetworkId; i <= now_max_sharding_id_; ++i) {
        ...
        SETH_DEBUG("sync net data from network: %u, hash64: %lu, src sharding id: %u",
...
void TxPoolManager::HandleSyncPoolsMaxHeight(const transport::MessagePtr& msg_ptr) {
    if (tx_pool_ == nullptr) {
        SETH_DEBUG("tx_pool_ == nullptr");
        return;
    }

    if (!msg_ptr->header.has_sync_heights()) {
        SETH_DEBUG("!msg_ptr->header.has_sync_heights()");
        return;
    }
    ...
    if (msg_ptr->header.sync_heights().req()) {
        ...
        if (src_net_id != local_des_shard_id) {
            if (src_net_id != network::kRootCongressNetworkId) {
                ...
                do {
                    ...
                    SETH_DEBUG("net: %u, get response pool heights, cross pool heights: %lu, update_height: %lu, "
                ...
                SETH_DEBUG("get root response pool heights: %s", sync_debug.c_str());
            }
            return;
        }
        ...
        SETH_DEBUG("get response pool heights: %s, cross pool heights: %s", sync_debug.c_str(), cross_debug.c_str());
    }
}
...
int32_t TxPoolManager::HandleElectTx(const transport::MessagePtr& msg_ptr) {
    ...
    if (msg_ptr->address_info->balance() < consensus::kJoinElectGas) {
        SETH_WARN("address info join elect gas invalid: %s %lu %lu", 
            common::Encode::HexEncode(addr).c_str(), 
    ...
    //         tx_msg.pubkey(),
    //         tx_msg.sign()) != security::kSecuritySuccess) {
    //     SETH_WARN("kElectJoin verify signature failed!");
    //     return;
    // }
...
bool TxPoolManager::UserTxValid(const transport::MessagePtr& msg_ptr) {
    ...
    if (msg_ptr->address_info->sharding_id() != local_shard) {
        SETH_WARN("sharding error id: %s, shard: %d, local shard: %d",
            common::Encode::HexEncode(msg_ptr->address_info->addr()).c_str(),
            msg_ptr->address_info->sharding_id(),
            common::GlobalInfo::Instance()->network_id());
        assert(false);
...
void TxPoolManager::DispatchTx(uint32_t pool_index, const transport::MessagePtr& msg_ptr) {
    ...
    if (item_functions_[msg_ptr->header.tx_proto().step()] == nullptr) {
        ...
        NotifyTxStatus(tx_status_cb_, msg_ptr, transport::kUnkonwn);
        return;
    }
    ...
    TMP_ADD_DEBUG_PROCESS_TIMESTAMP();
    SETH_DEBUG("trace tx success add local transfer to tx pool: %u, "
