...
int ShardStatistic::Init() {
    ...
    if (prefix_db_->GetLatestPoolStatisticTag(
        ...
        SETH_INFO("success set latest statisticed height: %lu, info: %s", 
        ...
        for (int32_t i = 0; i < statistic_info.pool_statisitcs_size(); ++i) {
            ...
            SETH_INFO("success set latest statisticed height "
        ...
        SETH_INFO("failed load latest pool statistic tag: %d", common::GlobalInfo::Instance()->network_id());
        assert(false);
        return kPoolsError;
    }
...
void ShardStatistic::ThreadToStatistic(
    ...
    SETH_INFO("new block coming net: %u, pool: %u, height: %lu, timeblock height: %lu",
    ...
    if (block_ptr->height() != pool_blocks_info->latest_consensus_height_ + 1) {
        ...
        SETH_INFO("pool latest height not continus: %u_%u_%lu, view: %lu, %lu,",
            view_block_ptr->qc().network_id(),
            view_block_ptr->qc().pool_index(),
            block_ptr->height(),
...
//     SETH_INFO("success handle statistic block: %s, latest_statisticed_height_: %lu, %d, %d",
...
//     auto& heights = elect_statistic.height_info();
    ...
    //     SETH_INFO("erase statistic height: %lu", st_iter->first);
...
void ShardStatistic::HandleStatistic(
    ...
    if (block.timeblock_height() < latest_timeblock_height_) {
        SETH_INFO("timeblock not less than latest timeblock: %lu, %lu", 
    ...
    if (block.has_pool_statistic_height() &&
        ...
        if (exist_iter == statistic_pool_info_.end()) {
            SETH_INFO(
                ...
                block.height() + 1,
                0);
            SETH_INFO("new timeblcok coming and should statistic new tx %lu, %lu.", 
    ...
    SETH_INFO("real handle new block coming net: %u, pool: %u, height: %lu, timeblock height: %lu",
        view_block_ptr->qc().network_id(),
        view_block_ptr->qc().pool_index(),
        block.height(),
    ...
    while (pool_statistic_riter != statistic_pool_info_.rend()) {
        ...
        SETH_INFO("check elect height: %lu, pool: %u, block height: %lu, find: %d",
        ...
        if (pool_iter != pool_statistic_riter->second.end()) {
            SETH_INFO("pool: %u, get block height and statistic height: %lu, max_height: %lu",
                pool_idx,
                block.height(),
    ...
    if (pool_statistic_riter == statistic_pool_info_.rend()) {
        SETH_INFO("statistic_pool_debug_str failed, has statisticed: %s", statistic_pool_debug_str.c_str());
        // assert(false);
        return;
    }
        ...
        for (auto iter = pool_statistic_height_with_block_height_map_.begin(); 
            ...
            if (iter->first > pool_statistic_riter->first) {
                ...
                if (tmp_pool_iter != iter->second.end()) {
                    ...
                    SETH_INFO("pool statistic set min and max height: %u, %lu, %lu, "
    ...
    auto handle_joins_func = [&](const bls::protobuf::JoinElectInfo& join_info) {
        SETH_INFO("join elect tx comming.");
        ...
        id_pk_map[join_addr] =join_info.public_key();
        {
            ...
            SETH_INFO("success add elect node stoke %s, %lu, "
...

        {
            ...
            SETH_INFO("kJoinElect add new elect node: %s, shard: %u, pool: %u, "
    ...
    if (block.has_elect_block()) {
        ...
        for(auto node : elect_block.in()) {
            ...
            SETH_INFO("HandleElectStatistic addr: %s, consensus_gap: %lu, credit: %lu",
    ...
    if (block.has_elect_statistic()) {
        ...
        for (int32_t node_idx = 0;
            ...
            SETH_INFO("success get shard election: %lu, %lu, "
            ...
            if (elect_statistic.join_elect_nodes(node_idx).shard() == network::kRootCongressNetworkId) {
                ...
                SETH_INFO("root sharding kJoinElect add new elect node: %s, "
    ...
    SETH_INFO("statistic height: %lu, success handle block pool: %u, height: %lu, "
...
std::string ShardStatistic::getLeaderIdFromBlock(
    ...
    auto members = elect_mgr_->GetNetworkMembersWithHeight(
        ...
        nullptr);
    if (members == nullptr) {
        SETH_INFO("block leader not exit block.hash %s block.electHeight:%d, network_id:%d ",
...
void ShardStatistic::CallNewElectBlock(
    ...
    SETH_INFO("new elect block: %lu, prepare_elect_height_: %lu",
...
int ShardStatistic::StatisticWithHeights(
    ...
    while (iter != statistic_pool_info_.rend() && iter->first > latest_statisticed_height_) {
        SETH_INFO("iter->first: %lu, size: %u, piter->first: %lu, size: %u, latest_statisticed_height_: %lu", 
            iter->first, 
            iter->second.size(), 
    ...
    if (piter == statistic_pool_info_.rend() || iter == statistic_pool_info_.rend()) {
        ...
        SETH_INFO("failed iter == statistic_pool_info_.end() piter: %d, iter: %d, "
    ...
    if (piter->second.size() != common::kInvalidPoolIndex ||
        ...
        SETH_INFO("pool not full statistic height: %lu, now: %u, all: %u, "
    ...
    for (uint32_t tmp_pool_idx = 0; tmp_pool_idx < common::kInvalidPoolIndex; ++tmp_pool_idx) {
        for (auto tmp_iter = pool_statistic_height_with_block_height_map_.begin(); 
            ...
            SETH_INFO("pool_statistic_height_with_block_height_map_ tmp_pool_idx: %d, "
            ...
            if (tmp_iter->first > iter->first) {
                ...
                if (tmp_pool_iter != tmp_iter->second.end()) {
                    ...
                    SETH_INFO("success get pool_statistic_height_with_block_height_map_ tmp_pool_idx: %d, "
                        "tmp_iter->first: %lu iter->first: %lu, tmp_pool_iter->second: %lu",
                        tmp_pool_idx, tmp_iter->first, iter->first, tmp_pool_iter->second);
                } else {
                    SETH_INFO("failed get pool_statistic_height_with_block_height_map_ tmp_pool_idx: %d, "
                        "tmp_iter->first: %lu iter->first: %lu",
                        tmp_pool_idx, tmp_iter->first, iter->first);
                }
        ...
        if (iter->second[tmp_pool_idx].statistic_min_height >
            ...
            SETH_INFO("pool: %d, statistic height: %lu, min height: %lu, max height: %lu",
                tmp_pool_idx,
                iter->first,
            ...
            for (auto tmp_iter = pool_statistic_height_with_block_height_map_.begin(); 
                ...
                auto tmp_pool_iter = tmp_iter->second.find(tmp_pool_idx);
                if (tmp_pool_iter != tmp_iter->second.end()) {
                    SETH_INFO("pool: %u, statistic height: %lu, block height: %lu",
    ...
    if (exist_iter != statistic_height_map_.end()) {
        ...
        SETH_INFO("success get exists statistic message iter->first: %lu, "
    ...
    if (now_elect_members == nullptr) {
        SETH_INFO("now_elect_members == nullptr.");
        return kPoolsError;
    }
    ...
    if (prepare_members != nullptr) {
        ...
        SETH_INFO("failed get prepare members prepare_elect_height_: %lu",
            static_cast<uint64_t>(prepare_elect_height_));
    }
    ...
    SETH_INFO("success create statistic message prev_timeblock_height_: %lu, "
...
void ShardStatistic::addPrepareMembers2JoinStastics(
    ...
    if (prepare_members != nullptr) {
        for (uint32_t i = 0; i < prepare_members->size(); ++i) {
            ...
            if (inc_iter != added_id_set.end()) {
                // SETH_INFO("id is in elect: %s", common::Encode::HexEncode(
            ...
            SETH_INFO("add node to election prepare member: %s, %s, stoke: %lu, shard: %u, elect pos: %d",
    ...
    if (prepare_members != nullptr) {
        SETH_INFO("kJoinElect add new elect node now elect_height: %lu, prepare elect height: %lu, "
...
void ShardStatistic::addNewNode2JoinStatics(
    ...
    for (auto iter = join_elect_stoke_map.begin(); iter != join_elect_stoke_map.end(); ++iter) {
        for (auto id_iter = iter->second.begin(); id_iter != iter->second.end(); ++id_iter) {
            SETH_INFO("stoke map eh: %lu, id: %s, stoke: %lu",
                iter->first,
    ...
    for (auto iter = join_elect_shard_map.begin(); iter != join_elect_shard_map.end(); ++iter) {
        for (auto id_iter = iter->second.begin(); id_iter != iter->second.end(); ++id_iter) {
            SETH_INFO("shard map eh: %lu, id: %s, shard: %u",
                iter->first,
                common::Encode::HexEncode(id_iter->first).c_str(),
                id_iter->second);
        }
    ...
    if (r_eiter != join_elect_stoke_map.rend() &&
        ...
        for (auto &[node_id, stoke] : stoke_map) {
            if (shard_map.count(node_id) == 0) {
                SETH_INFO("failed get shard: %s", common::Encode::HexEncode(node_id).c_str());
                continue;
            }

            if (added_id_set.count(node_id) > 0) {
                // 说明节点是之前的委员会成员 。
                SETH_INFO("not added id: %s", common::Encode::HexEncode(node_id).c_str());
                continue;
            }

            elect_nodes.push_back(node_id);
            SETH_INFO("elect nodes add: %s, %lu",
    ...
    for (uint32_t i = 0; i < elect_nodes.size() && i < kWaitingElectNodesMaxCount; ++i) {
        ...
        join_elect_node->set_elect_pos(0);
        SETH_INFO("add node to election new member: %s, %s, stoke: %lu, shard: %u, elect pos: %d",
                  common::Encode::HexEncode(pubkey).c_str(),
                  common::Encode::HexEncode(secptr_->GetAddress(pubkey)).c_str(),
                  iter->second, shard_iter->second,
                  0);
        SETH_INFO("add new elect node: %s, stoke: %lu, shard: %u",
            common::Encode::HexEncode(pubkey).c_str(), iter->second, shard_iter->second);
    }
}

void ShardStatistic::setElectStatistics(
        std::map<uint64_t, std::unordered_map<std::string, seth::pools::StatisticMemberInfoItem>> &height_node_collect_info_map,
        seth::common::MembersPtr &now_elect_members,
        seth::pools::protobuf::ElectStatistic &elect_statistic,
        bool is_root) {
