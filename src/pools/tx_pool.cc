...
// 
    if (latest_height_ == common::kInvalidUint64) {
        ...
        SETH_DEBUG("get invalid heights size: %u, latest_height_: %lu", 0, latest_height_);
        return 0;
    }
...
void TxPool::TxOver(view_block::protobuf::ViewBlockItem& view_block) {
    ...
    for (uint32_t i = 0; i < (uint32_t)view_block.block_info().tx_list_size(); ++i) {
        ...
        if (addr.empty()) {
            SETH_DEBUG("addr is empty: %s", ProtobufToJson(view_block.block_info().tx_list(i)).c_str());
            assert(false);
            continue;
        }
        ...
        auto remove_tx_func = [&](std::map<std::string, std::map<uint64_t, TxItemPtr>>& tx_map) {
            ...
            if (tx_iter != tx_map.end()) {
                for (auto nonce_iter = tx_iter->second.begin(); nonce_iter != tx_iter->second.end(); ) {
                    SETH_DEBUG("find tx addr success: %s, unique hash: %s, "
                    ...
                    SETH_DEBUG("find tx addr success: %s, nonce: %lu, remove nonce: %lu", 
                        common::Encode::HexEncode(addr).c_str(), 
                        nonce_iter->first,
                        view_block.block_info().tx_list(i).nonce());
                    if (!IsUserTransaction(view_block.block_info().tx_list(i).step())) {
                    ...
                    SETH_DEBUG("over pop success add system tx nonce addr: %s, "
                ...
                SETH_DEBUG("find tx addr failed: %s", common::Encode::HexEncode(addr).c_str());
            }
        };
...
void TxPool::GetTxSyncToLeader(
    ...
    while (added_txs_.pop(&tx_ptr)) {
        SETH_DEBUG("pop success add system tx nonce addr: %s, addr nonce: %lu, tx nonce: %lu, unique hash: %s",
                common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
                tx_ptr->address_info->nonce(), 
                tx_ptr->tx_info->nonce(),
                common::Encode::HexEncode(tx_ptr->tx_info->key()).c_str());
         if (!IsUserTransaction(tx_ptr->tx_info->step())) {
            ...
            SETH_DEBUG("success add system tx nonce addr: %s, addr nonce: %lu, tx nonce: %lu, unique hash: %s",
                common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
                tx_ptr->address_info->nonce(), 
                tx_ptr->tx_info->nonce(),
                common::Encode::HexEncode(tx_ptr->tx_info->key()).c_str());
        ...
        if (iter != consensus_tx_map_.end()) {
            ...
            if (nonce_iter != iter->second.end()) {
                SETH_DEBUG("exists failed add tx nonce invalid addr: %s, addr nonce: %lu, tx nonce: %lu",
                    common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
                    tx_ptr->address_info->nonce(), 
                    tx_ptr->tx_info->nonce());
                continue;
        ...
        SETH_DEBUG("success add tx nonce invalid addr: %s, addr nonce: %lu, tx nonce: %lu",
            common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(),
            tx_ptr->address_info->nonce(), 
            tx_ptr->tx_info->nonce());
    }
...
void TxPool::InitLatestInfo() {
    ...
    if (prefix_db_->GetLatestPoolInfo(
        ...
        if (latest_height_ == common::kInvalidUint64 || latest_height_ < pool_info.height()) {
            ...
            SETH_DEBUG("init latest pool info shard: %u, pool %lu, init height: %lu",
...
uint64_t TxPool::UpdateLatestInfo(
    ...
    SETH_DEBUG("pool index: %d, new height: %lu, new synced height: %lu, prev_synced_height_: %lu, to_sync_max_height_: %lu, latest height: %lu",
...
void TxPool::ConsensusAddTxs(const pools::TxItemPtr& tx_ptr) {
    ...
    SETH_DEBUG("trace tx pool: %d, sync add tx addr: %s, nonce: %lu", 
        pool_index_,
        common::Encode::HexEncode(tx_ptr->address_info->addr()).c_str(), 
        tx_ptr->tx_info->nonce());
