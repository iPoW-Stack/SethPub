...
void PkiIbAgka::Setup() {
  ...
  // k_.set_random();
  {
      ...
      auto hex_bytes = seth::common::Encode::HexEncode(k_.to_bytes());
      std::cout << hex_bytes << std::endl;
      k_.from_bytes(seth::common::Encode::HexDecode(hex_bytes));
      std::cout << seth::common::Encode::HexEncode(k_.to_bytes()) << std::endl;
  }
  ...
  // pp.g.set_random();
  {
      ...
      auto hex_bytes = seth::common::Encode::HexEncode(pp.g.to_bytes());
      std::cout << hex_bytes << std::endl;
      pp.g.from_bytes(seth::common::Encode::HexDecode(hex_bytes));
      std::cout << seth::common::Encode::HexEncode(pp.g.to_bytes()) << std::endl;
  }
...
int PkiIbAgka::PkiExtract(
  ...
  std::string tmp_value = sk_str + "," + seth::common::Encode::HexEncode(pk.to_bytes());
  param.zjc_host->SaveKeyValue(param.from, tmp_key, tmp_value);
  SETH_DEBUG("success pki extract index: %d key: %s, value: %s", i, tmp_key.c_str(), tmp_value.c_str());
  return 0;
}
...
void PkiIbAgka::PkiExtract(const int& n) {
  ...
  for (auto i = 0; i < n; i++) {
    ...
    sk.set_random();
    {
        ...
        auto hex_bytes = seth::common::Encode::HexEncode(sk.to_bytes());
        std::cout << hex_bytes << std::endl;
        sk.from_bytes(seth::common::Encode::HexDecode(hex_bytes));
        std::cout << seth::common::Encode::HexEncode(sk.to_bytes()) << std::endl;
    }
...
int PkiIbAgka::EncKeyGen(
    ...
    if (!common::StringUtil::ToInt32(lines[1], &ib_count)) {
        SETH_DEBUG("ib_count count error: %s", lines[1]);
        return 1;
    }
    ...
    if (ib_count < 2 || ib_count >= 1024) {
        SETH_DEBUG("ib_count count error: %d", ib_count);
        return 1;
    }
    ...
    for (int32_t i = 0; i < pki_count; ++i) {
      ...
      G1 sk(pp.e);
      sk.from_bytes(seth::common::Encode::HexDecode(val_splits[0]));
      G2 pk(pp.e);
      pk.from_bytes(seth::common::Encode::HexDecode(val_splits[1]));
    ...
    std::string tmp_value = seth::common::Encode::HexEncode(omega.to_bytes()) + "," + 
      seth::common::Encode::HexEncode(Q.to_bytes());
    param.zjc_host->SaveKeyValue(param.from, tmp_key, tmp_value);
...
int PkiIbAgka::DecKeyGen(
        ...
        const std::string& in_key, 
        const std::string& value) {
    SETH_DEBUG("called dec key gen %s", value.c_str());
    auto lines = common::Split<>(value.c_str(), ';');
    if (lines.Count() != 3) {
        SETH_DEBUG("line count error: %d", lines.Count());
        return 1;
...
int PkiIbAgka::Enc(
    ...
    SETH_DEBUG("enc  ek.omega: %s, c2: %s", 
    ...
    SETH_DEBUG("enc  ek.q: %s, tmp1: %s, tmp2: %s, c3: %s", 
...
CipherText PkiIbAgka::Enc(PlainText& plain, EncodeKey& ek) {
  ...
  e.set_random();
  {
      ...
      auto hex_bytes = seth::common::Encode::HexEncode(e.to_bytes());
      std::cout << hex_bytes << std::endl;
      e.from_bytes(seth::common::Encode::HexDecode(hex_bytes));
      std::cout << seth::common::Encode::HexEncode(e.to_bytes()) << std::endl;
  }
  // calc c1 = g^e
  G1 c1(pp.e);
  c1 = pp.g.pow_zn(e);
