            ...
            if (consensus_thread_index_map_[i] == common::kInvalidUint8) {
                ...
                if (i == message_handler_thread_count_ - 1) {
                    ...
                    for (uint8_t src_thread_idx = 0; src_thread_idx < i; ++src_thread_idx) {
                        ...
                        SETH_DEBUG("thread: %d, src_thread_idx: %d, pools: %s", 
                ...
                SETH_INFO("thread index %d set cosensus index: %d", thread_idx, i);
                return i;
            }
        }

        SETH_FATAL("invalid thread idx: %d, bft_thread: %d", thread_idx, bft_thread);
