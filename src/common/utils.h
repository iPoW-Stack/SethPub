...
#define ADD_DEBUG_PROCESS_TIMESTAMP() { \
    if (msg_ptr) { \
        ...
        if (msg_ptr->times_idx > 0) { diff_time = btime - msg_ptr->times[msg_ptr->times_idx - 1]; if (diff_time > 20000lu)SETH_INFO("over handle message debug use time: %lu, type: %d", diff_time, msg_ptr->header.type());} \
...
//         auto btime = common::TimeUtils::TimestampUs(); \
//         uint64_t diff_time = 0; \
//         if (msg_ptr->times_idx > 0) { diff_time = btime - msg_ptr->times[msg_ptr->times_idx - 1]; if (diff_time > 10000lu)SETH_INFO("over handle message debug use time: %lu, type: %d", diff_time, msg_ptr->header.type());} \
//         msg_ptr->debug_str[msg_ptr->times_idx] = std::string(SETH_LOG_FILE_NAME) + ":" + std::to_string(__LINE__); \
//         msg_ptr->times[msg_ptr->times_idx] = btime; \
//         msg_ptr->times_idx++; \
//     } \
...
static const uint32_t kMaxTxCount = 12000u;
...
static const int64_t kRotationPeriod = 120000ll * 1000ll * 1000ll; // for quicker debugging
