...
//     evm_ = evmc::VM{ evmc_load_and_configure("/root/seth/third_party/evmone/build/lib64/libevmone.so", &ec)};
...
int Execution::execute(
    ...
    if (code_size <= kContractHead.size() ||
        ...
        SETH_DEBUG("invalid params code_size: %u, from size: %u, "
    ...
    SETH_DEBUG("now call contract, msg sender: %s, mode: %d, from: %s, to: %s, value: %lu, bytes_code.size: %ld, input: %s",
