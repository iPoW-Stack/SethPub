...
class SethhainHost : public evmc::Host {
public:
    SethhainHost() {
    ...
    void MergeToPrev() {
        for (auto iter = recorded_logs_.begin(); iter != recorded_logs_.end(); ++iter) {
            pre_seth_host_->recorded_logs_.push_back(*iter);
        }

        for (auto iter = to_account_value_.begin(); iter != to_account_value_.end(); ++iter) {
            pre_seth_host_->to_account_value_[iter->first] = iter->second;
        }

        for (auto iter = accounts_.begin(); iter != accounts_.end(); ++iter) {
            auto old_iter = pre_seth_host_->accounts_.find(iter->first);
            if (old_iter == pre_seth_host_->accounts_.end()) {
                pre_seth_host_->accounts_[iter->first] = iter->second;
                continue;
            }
        ...
        for (auto iter = account_balance_.begin(); iter != account_balance_.end(); ++iter) {
            pre_seth_host_->account_balance_[iter->first] = iter->second;
        }
