#!/bin/bash
# ============================================================
# Seth 区块链 Docker 分布式一键编译部署脚本
#
# 功能:
#   1. 本地构建 seth-builder 编译镜像并编译二进制
#   2. 本地 genesis 初始化，生成 shards/shard_db/pkg
#   3. 生成 bootstrap 字符串，填充 seth.conf 占位符
#   4. 将 pkg 分发到各远端主机
#   5. 各远端主机为每个节点启动独立 Docker 容器
#   6. 放通 HTTP / TCP / WebSocket 端口
#
# 用法:
#   bash seth_docker_deploy.sh -H <IP列表> [选项]
#
# 选项:
...
#   -r REPO_DIR         seth p2p 源码目录（使用 -b 时可省略）
#   -R REBUILD          强制重建镜像 0/1（默认: 0）
#   -b SETH_BIN         已有 seth 二进制路径（跳过编译，直接用此文件做 genesis）
#   -d DATA_DIR         节点数据根目录（默认: /data，容器 volume 挂载此路径下 nodes/）
#   -h                  显示帮助
#
# 示例:
#   # 单机: shard 2-6, 每分片 4 节点, 共 20 容器（使用已有二进制）
#   bash seth_docker_deploy.sh -H 192.168.25.129 -n 4 -s 2 -N 5 -b /root/seth/cbuild_Release/seth
#
#   # 单机: shard 2-4, 每分片 4 节点, 共 12 容器（自动编译）
#   bash seth_docker_deploy.sh -H 192.168.25.129 -n 4 -s 2 -N 3
#
#   # 20 台主机, 4 个分片(2-5), 每台每分片 4 节点
#   # => 每分片 5台*4节点=20节点, 总节点=80
#   bash seth_docker_deploy.sh \
...
SSH_PORT=22
REBUILD=0
SETH_BIN=""    # -b: 已有二进制，跳过编译
NODES_DATA_DIR="/data"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_DIR="${SCRIPT_DIR}/../../p2p"
BUILDER_IMAGE="seth-builder:latest"
RUNTIME_IMAGE="seth-runtime:latest"
...
if [ -n "$SETH_BIN" ]; then
    [ -f "$SETH_BIN" ] || { echo "错误: -b 指定的 seth 二进制不存在: $SETH_BIN" >&2; exit 1; }
    SETH_BIN="$(cd "$(dirname "$SETH_BIN")" && pwd)/$(basename "$SETH_BIN")"
else
...
step1_build_builder_image() {
    [ -z "$SETH_BIN" ] || { echo "[1/5] 跳过编译镜像（已指定 -b $SETH_BIN）"; return 0; }
    echo ""
...
step3_genesis_and_package() {
    ...
    if [ -n "$SETH_BIN" ]; then
        ...
        # 拷贝 seth 所需的辅助文件（conf/、log4cpp、mmdb、temp 目录）
        local seth_dir; seth_dir="$(dirname "$SETH_BIN")"
        local seth_root; seth_root="$(dirname "$seth_dir")"  # cbuild_Release 的上层即 p2p 根
        mkdir -p "$wdir/conf"
        [ -d "$seth_root/nodes_local/seth/conf" ] && \
            cp -r "$seth_root/nodes_local/seth/conf/." "$wdir/conf/" 2>/dev/null || true
        [ -d "$seth_root/nodes_local/temp" ] && \
            cp -r "$seth_root/nodes_local/temp" "$wdir/temp" 2>/dev/null || true

        ( cd "$wdir"
          echo "--- genesis -U ---"
          "$SETH_BIN" -U -N "$GENESIS_NODE_COUNT" -E $(( END_SHARD + 1 ))
          echo "--- genesis -S ---"
          "$SETH_BIN" -S 3 -N "$GENESIS_NODE_COUNT" -E $(( END_SHARD + 1 ))
          echo "--- genesis -C ---"
          "$SETH_BIN" -C
        )
        ...
        mkdir -p "$PKG_DIR"
        cp "$SETH_BIN"                              "$PKG_DIR/seth"
        chmod +x "$PKG_DIR/seth"
        ...
        for shard_id in $(seq "$START_SHARD" "$END_SHARD"); do
            ...
            "$SETH_BIN" -A "$sf" -D "$PKG_DIR/shards${shard_id}"
...
lines = [
    ...
    'cp /root/seth/nodes_local/seth/conf/log4cpp.properties "$NODES_DIR/conf/" 2>/dev/null || true',
    'cp /root/seth/nodes_local/seth/conf/GeoLite2-City.mmdb "$NODES_DIR/conf/" 2>/dev/null || true',
    'cp -r /root/seth/nodes_local/temp "$NODES_DIR/temp"',
    'ln -sf "$SETH" "$NODES_DIR/seth"',
    'cd "$NODES_DIR"',
    'echo "--- genesis -U ---"',
    '"$SETH" -U -N "$GENESIS_NODE_COUNT" -E $(( END_SHARD + 1 ))',
    'echo "--- genesis -S ---"',
    '"$SETH" -S 3 -N "$GENESIS_NODE_COUNT" -E $(( END_SHARD + 1 ))',
    'echo "--- genesis -C ---"',
    '"$SETH" -C',
    'cp "$NODES_DIR/seth"                         "$PKG_DIR/seth"',
    ...
    '    "$SETH" -A "$sf" -D "$PKG_DIR/shards${shard_id}"',
    '    cp "$sf" "$PKG_DIR/init_accounts${shard_id}"',
...
step4_configure_nodes() {
    ...
    for shard_id in $(seq "$START_SHARD" "$END_SHARD"); do
        ...
        for (( i=1; i<=total_shard_nodes; i++ )); do
            ...
            local conf="${node_dir}/conf/seth.conf"
            ...
            openssl req -x509 -newkey rsa:2048 -nodes \
                ...
                -days 365 -subj "/C=CN/O=Seth/CN=node" 2>/dev/null
            chmod 600 "${node_dir}/server-key.pem"
        done
    done

    # 打成一个 tar 供分发
    PKG_TAR="$PKG_WORK/seth_pkg.tar.gz"
...
step5_deploy_to_hosts() {
    ...
    # 确保远端有 seth-runtime 镜像（导出/导入或 docker pull）
    # 先把本地镜像保存成 tar，再 scp 到远端加载
    local img_tar="$PKG_WORK/seth_runtime.tar"
    ...
    for shard_id in $(seq "$START_SHARD" "$END_SHARD"); do
        ...
        for ip in "${hosts[@]}"; do
            ...
            (
                # 1. 上传镜像（如不是本机）
                if ! is_local "$ip"; then
                    scp_to "$ip" "$img_tar" "/tmp/seth_runtime.tar"
                    ssh_run "$ip" "docker load -i /tmp/seth_runtime.tar && rm -f /tmp/seth_runtime.tar"
                fi

                # 2. 上传 pkg tar
                scp_to "$ip" "$PKG_TAR" "/tmp/seth_pkg.tar.gz"

                # 3. 解包到数据盘
                ssh_run "$ip" "
                    mkdir -p ${NODES_DATA_DIR}
                    rm -rf ${NODES_DATA_DIR}/seths ${NODES_DATA_DIR}/pkg ${NODES_DATA_DIR}/nodes
                    tar -xzf /tmp/seth_pkg.tar.gz -C ${NODES_DATA_DIR}/
                    rm -f /tmp/seth_pkg.tar.gz
                "
                ...
                ssh_run "$ip" "
                    for d in ${NODES_DATA_DIR}/nodes/s${shard_id}_*; do
                        ...
                        openssl req -x509 -newkey rsa:2048 -nodes \
                            ...
                            -days 365 -subj \"/C=CN/O=Seth/CN=${ip}\" 2>/dev/null
                        chmod 600 \"\$d/server-key.pem\"
                    done
                "
                # 也用真实 IP 更新 seth.conf 中的 public_ip / local_ip
                ssh_run "$ip" "
                    for conf in ${NODES_DATA_DIR}/nodes/s${shard_id}_*/conf/seth.conf; do
                        [ -f \"\$conf\" ] || continue
                ...
                # 5. 停并删除同分片旧容器
                ssh_run "$ip" "
                    docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^seth-${shard_id}-' | while read name; do
                ...
                for (( i=host_start; i<=host_end; i++ )); do
                    local instance="s${shard_id}_${i}"
                    local container="seth-${shard_id}-${i}"
                    local tcp_port http_port ws_port
                    tcp_port=$(calc_port  "$shard_id" "$i" "1")
                    http_port=$(calc_port "$shard_id" "$i" "2")
                    ws_port=$(calc_port   "$shard_id" "$i" "3")
    ...
    for shard_id in $(seq "$START_SHARD" "$END_SHARD"); do
        ...
        for ip in "${hosts[@]}"; do
            for (( j=0; j<NODES_PER_HOST; j++ )); do
                ...
                pid=$(ssh_run "$ip"    "docker exec ${container} pgrep -x seth 2>/dev/null || true" 2>/dev/null || true)
