...
bash "${SETH_ROOT}/scripts/rebuild_uSockets_nolto.sh"
...
echo "=== 3/3 Clean link of seth and rebuild ==="
# Drop stale .o built with -flto from a previous configure
find "${BUILD_DIR}" -name '*.o' -path '*/CMakeFiles/seth.dir/*' -delete 2>/dev/null || true
cmake --build . --target seth -j"$(nproc 2>/dev/null || echo 4)"

echo "OK: ${BUILD_DIR}/seth"
