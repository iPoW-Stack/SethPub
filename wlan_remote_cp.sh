...
scp_package() {
    ...
    for ip in "${node_ips_array[@]}"; do 
        ...
        sshpass -p $PASSWORD scp -o ConnectTimeout=10  -o StrictHostKeyChecking=no /root/seth/cp_pkg.sh root@$ip:/root &
        sshpass -p $PASSWORD scp -o ConnectTimeout=10  -o StrictHostKeyChecking=no /root/seth/sshpass root@$ip:/usr/bin &
        run_cmd_count=$((run_cmd_count + 1))
        if (($run_cmd_count >= 10)); then
            check_cmd_finished
            run_cmd_count=0
