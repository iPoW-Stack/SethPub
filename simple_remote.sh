...
run_command() {
    ...
    for ip in "${node_ips_array[@]}"; do
        ...
        sshpass -p $PASSWORD ssh -o ConnectTimeout=3 -o "StrictHostKeyChecking no" -o ServerAliveInterval=5  root@$ip -p 22 "cd /root && tar -zxvf pkg.tar.gz && cd ./pkg && bash temp_cmd.sh $ip $start_pos $start_nodes_count 0 2 $end_shard $leader_init_tm 'ech0'"  > /dev/null 2>&1 &
        if ((start_pos==1)); then
            sleep 3
        fi
