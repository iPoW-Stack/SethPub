...
deploy_nodes() {
    ...
    for ((shard_id=$start_shard; shard_id<=$end_shard; shard_id++)); do
        ...
        for ((i=$start_pos; i<=$end_pos; i++)); do
            ...
            inst_dir="/root/seths/s${shard_id}_${i}"
            if [ -d "$inst_dir" ]; then
            ...
            sed -i "s/PRIVATE_KEY/${prikey}/g" "$inst_dir/conf/seth.conf"
            sed -i "s/PUBLIC_IP/${public_ip}/g" "$inst_dir/conf/seth.conf"
            sed -i "s/LOCAL_IP/${local_ip}/g" "$inst_dir/conf/seth.conf"
            sed -i "s/LEADER_CHANGE_INIT_TM/${leader_init_tm}/g" "$inst_dir/conf/seth.conf"
            if [ "$shard_id" -eq 3 ]; then
                if ((i <= TEST_TX_MAX_POOL_INDEX)); then
                    sed -i "s/TEST_POOL_INDEX/$(($i - 1))/g" "$inst_dir/conf/seth.conf"
                else
                    sed -i "s/TEST_POOL_INDEX/-1/g" "$inst_dir/conf/seth.conf"
                fi
                sed -i "s/TEST_TX_TPS/${TEST_TX_TPS}/g" "$inst_dir/conf/seth.conf"
            fi
            ...
            sed -i "s/HTTP_PORT/${port1}/g" "$inst_dir/conf/seth.conf"
            sed -i "s/LOCAL_PORT/${port0}/g" "$inst_dir/conf/seth.conf"
            sed -i "s/TX_WS_PORT/${port2}/g" "$inst_dir/conf/seth.conf"

            echo "${inst_dir}/seth"

            rm -f "$inst_dir/seth" "$inst_dir/txcli"
