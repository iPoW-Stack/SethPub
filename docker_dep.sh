...
source ~/.bash_profile && cd /root/seth && sh build.sh a Debug && sh build.sh a Release && docker build -t $IMAGE_NAME:latest . && rm -rf $IMAGE_NAME.tar && docker save -o $IMAGE_NAME.tar $IMAGE_NAME&& ctr -n k8s.io images import $IMAGE_NAME.tar
